...
<p align="center">
  <a href="https://github.com/gulagala001/oh-my-dsh/releases/tag/v0.1.7-rc.2.12"><img src="https://img.shields.io/badge/version-0.1.7--rc.2.11-3478F6?style=flat-square" alt="Version 0.1.7-rc.2.12" /></a>
...
<p align="center">
  ...
  <a href="https://github.com/gulagala001/oh-my-dsh/releases/tag/v0.1.7-rc.2.12">版本说明</a>
</p>
...
**0.1.7-rc.2.12 正式版，适配 DSH 0.1.7-rc.2 Web 与官方桌面版，内置 OpenCU 1.1.6。**

<a id="quickstart"></a>

## 装上，开始工作

版本规则：`DSH 完整版本.插件补丁号`，例如 `0.1.7-rc.2.1` → `0.1.7-rc.2.12`。旧 1.x 检查器不能自动识别这次编号切换，请按下方新 tag 手动更新一次；历史 tag 保留。

### 自己安装 Web 版

本版配对 **DSH 0.1.7-rc.2 + Oh My DSH 0.1.7-rc.2.12**。已有环境先等待任务结束，停止服务并完整备份实际 `DSH_HOME`；继续使用原数据目录、profile 和端口。首次安装可直接运行：

```sh
npx --yes @deepseek-ai/dsh@0.1.7-rc.2 plugin --profile web add github:gulagala001/oh-my-dsh#v0.1.7-rc.2.12
...
2. 打开应用，按引导配置模型，在「插件」页面安装 `github:gulagala001/oh-my-dsh#v0.1.7-rc.2.12` 并确认启用。
3. 完整退出应用后重新打开，再新建会话选择 **Oh My DSH**，检查插件「当前版本」为 **0.1.7-rc.2.12**。

桌面版使用独立的 `desktop` profile；Web 已安装的插件需要在桌面端再安装一次。不要使用 CLI 管理 `desktop` profile。已有环境升级前先等任务结束、完整退出并备份实际 `DSH_HOME`。[桌面验证范围](docs/release-0.1.7-rc.2.12.md#验证范围)。
...
**0.1.7-rc.2.12**：修复官方桌面会话区仍被底色遮住壁纸的问题，三个壁纸区域与面板不透明度均正确生效；保留高级外观、Logo 与浏览器标题定制。

[更新记录](CHANGELOG.md) · [本版验证与升级说明](docs/release-0.1.7-rc.2.12.md) · [下载与校验文件](https://github.com/gulagala001/oh-my-dsh/releases/tag/v0.1.7-rc.2.12)
