...
当前 main：空闲预处理新增独立开关与等待时间，默认关闭；图片接近请求上限时批量卸载旧图并预留余量，保留原始附件。CoT 前置机制不变。
...
| `/compact-f` | 将当前完整对话、用户消息、工具结果、旧摘要和前置 Trace 重新汇总为一份摘要；不保留近期原文窗口 | 是，使用中枢的后台模型配置 |

全量模式保留系统提示词、当前工具定义和用户手写全局背景，不删除原始日志或文档存档。图片等非文本内容保留原始事件位置，不凭空生成视觉描述。全量模式可能损失细节；失败或摘要未缩短时不替换原文，写入中断使用现有事务恢复。

执行中的会话会将命令排队，在下一次请求边界执行；侧栏显示排队、生成中、失败和完成状态。手动压缩会取消本会话未完成的旧预处理/中枢请求，防止旧结果覆盖手动选择。历史全量压缩过的用户原文和 Trace 不会在后续整理时重新灌入。
