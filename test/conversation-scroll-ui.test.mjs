...
test('operation details stay inside the drawer without extending the conversation scroll range', { timeout: 90000 }, async t => {
  ...
  const group = page.locator('[data-cu-group] .tx-cu-group-toggle').filter({ hasText: '24 次操作' });
  await group.click();
  const list = page.locator('[data-step-process-body]:visible').filter({ has: page.locator('[data-chat-call-id="scroll-command-1"]') });
  ...
  await list.hover(); await page.mouse.wheel(0, 80);
  await until(async () => await list.evaluate(el => el.scrollTop) > 100);
  assert.equal(await scroll.evaluate(el => el.scrollTop), beforeWheel, 'wheel scroll stays inside the native drawer before its edge');
