...
test('DSH frontend: one workbench, preserved edits, compact composer and both themes', { timeout: 90000 }, async t => {
  ...
  const idleSwitch = settings.getByRole('switch', { name: '空闲时自动预处理', exact: true });
  const idleDelay = settings.getByLabel('空闲等待时间 · 秒', { exact: true });
  assert.equal(await idleSwitch.isChecked(), false); assert.equal(await idleDelay.isDisabled(), true);
  await idleSwitch.check(); await idleDelay.fill('45');
  ...
  assert.equal(await idleSwitch.isChecked(), true); assert.equal(await idleDelay.inputValue(), '45');
  await idleSwitch.uncheck();
  await settings.getByRole('button', { name: '保存设置', exact: true }).click();
  await until(async () => !(await settings.getByRole('button', { name: '保存设置', exact: true }).isEnabled()));
  assert.equal(await idleDelay.isDisabled(), true); assert.equal(await idleDelay.inputValue(), '45');
  ...
  await settings.getByRole('button', { name: '常用', exact: true }).click();
  await settings.getByRole('button', { name: '频繁', exact: true }).click();
  ...
  await settings.getByRole('button', { name: '保存设置', exact: true }).click();
  await until(async () => !(await settings.getByRole('button', { name: '保存设置', exact: true }).isEnabled()));
  await settings.getByRole('button', { name: '高级', exact: true }).click();
