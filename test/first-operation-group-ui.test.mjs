...
test('first operation group after user input stays visible with an injected runtime record', { timeout: 60000 }, async t => {
  ...
  await until(() => step === 3);
  const first = page.locator('[data-cu-group] > button').filter({ hasText: '2 次操作' });
  const later = page.locator('[data-cu-group] > button').filter({ hasText: '1 次操作' });
  await first.waitFor();
  await later.waitFor();
  assert.ok(await page.locator('[data-omd-record=injection]').count(), 'fixture includes the actual injected runtime record');
  await later.click();
  ...
  assert.equal(await page.locator('[data-cu-operation="first-operation-0"]').isVisible(), true);
  assert.equal(await page.locator('[data-cu-operation="first-operation-1"]').isVisible(), true);
  const duplicates = page.locator('.tx-cu-group-item:has([data-omd-record-hidden])');
  assert.ok(await duplicates.count(), 'the first group contains a duplicate of the separately displayed runtime record');
  assert.equal(await duplicates.filter({ visible: true }).count(), 0, 'only the duplicate record is hidden');
  ...
  const imageRow = page.locator('[data-cu-operation="first-operation-1"]');
