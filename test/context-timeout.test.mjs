...
test('default background deadline does not abort at two minutes and aborts at ten', async t => {
  ...
  const hub = Object.assign(Object.create(Hub.prototype), {
    ...
    ctx: { llm: { stream() { return { [Symbol.asyncIterator]() { return this; }, next: () => new Promise(() => {}), return: async () => ({ done: true }) }; } } },
  });
  ...
  assert.equal(records.length, 1); assert.equal(records[0].durationMs, 600000); assert.equal(hub.live.size, 0);
});
