...
function compactGuestLocations(text: string): string {
  // Encoded JavaScript still contains literal parentheses. Stop at whitespace,
...
export function renderThrown(error: unknown): string {
  try {
    ...
    if (typeof stack === 'string' && stack.length > 0) {
      // Node embeds the complete encoded guest module in data-URL stack
      // frames. Keep the location without sending that source back to the model.
