...
if (execFileSync('git', ['rev-parse', 'HEAD'], { cwd: source, encoding: 'utf8' }).trim() !== commit) throw new Error('Expected the pinned DSH 0.1.7-alpha.2 source checkout');
...
Object.assign(pkg, { name: '@oh-my-dsh/ui-conversation', version: '0.1.7-alpha.2-omd.1', private: true,
...
await writeFile(join(root, 'vendor/dsh.json'), JSON.stringify({ repository: 'https://github.com/deepseek-ai/deepseek-harness', tag: 'dsh-v0.1.7-alpha.2', commit, files: Object.fromEntries(Object.entries(files).sort()) }, null, 2) + '\n');
