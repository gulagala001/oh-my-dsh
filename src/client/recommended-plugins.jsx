  ...
  return <section className="tx-app tx-recommended" aria-labelledby={headingId}>
    ...
    {plugins.length > 0 ? <>
      ...
      {matches.length > 0 ? <div className="tx-recommended-grid">{matches.map(plugin => { const installed = state?.plugins.find(item => item.id === plugin.id), busy = state?.busy?.id === plugin.id ? state.busy : null;
        ...
        <footer>{plugin.category && <span className="tx-badge">{plugin.category}</span>}{plugin.upstream && <a className="tx-recommended-link" href={plugin.upstream.url} target="_blank" rel="noopener noreferrer" aria-label={`查看原作 ${plugin.upstream.name}（新窗口）`}>原作者项目<PluginIcon kind="arrow" size={13}/></a>}<a className="tx-recommended-link" href={plugin.url} target="_blank" rel="noopener noreferrer" aria-label={`查看 ${plugin.name} 项目（新窗口）`}>查看项目<PluginIcon kind="arrow" size={13}/></a></footer>
