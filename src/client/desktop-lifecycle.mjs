...
export function installDesktopLifecycle(ctx) {
  ...
  const bootEnabled = window.__DSH_BOOT__?.entries?.some(row => row.id === 'trisoul_x') === true;
  root.effect(() => {
    root[installed] = true;
    let notice;
    const sync = () => {
      if (modules.entries.state.getSnapshot().syncing) return;
      const enabled = modules.manifest.plugins.some(row => row.id === 'trisoul_x');
      if (enabled === bootEnabled) return;
      if (notice) return;
      notice = document.createElement('div');
      notice.dataset.omdDesktopRestart = '';
      notice.setAttribute('role', 'status');
      notice.textContent = 'Oh My DSH 的安装或启停已保存。请使用应用菜单中的「重启应用与 Host」完成切换；仅刷新页面不能生效。';
