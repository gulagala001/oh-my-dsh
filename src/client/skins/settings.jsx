...
function AppearanceSettings({ runtime }) {
  ...
  const importFile = async event => {
    ...
    if (!file) return;
    try {
      if (file.size > MAX_SKIN_BYTES) throw new Error('皮肤包不能超过 1 MB');
      runtime.import(JSON.parse(await file.text())); setError('');
    } catch (e) { setError(e instanceof SyntaxError ? '皮肤文件不是有效的 JSON' : e.message); }
  };
  ...
  return <section className="tx-app tx-recommended omd-appearance">
    <div className="tx-recommended-heading"><div><h2>外观</h2><p>皮肤即时生效，保存在当前浏览器。明暗模式与通用设置保持同步。</p></div></div>
    <label className="omd-appearance-row">皮肤<select aria-label="皮肤" value={state.selected} onChange={e => act(() => runtime.select(e.target.value))}>
    ...
    <label className="omd-appearance-row">导入皮肤<input aria-label="导入皮肤" type="file" accept=".json,application/json" onChange={importFile}/></label>
    <p className="omd-appearance-help">选择 .omd-skin.json 文件；导入同一皮肤会更新已有版本。</p>
    <div className="omd-appearance-actions"><button type="button" className="tx-button" onClick={() => act(() => runtime.reset())}>恢复默认皮肤</button>
      {state.selected !== 'default' && !state.skins.find(s => s.id === state.selected)?.builtin && <button type="button" className="tx-button" onClick={() => act(() => runtime.remove(state.selected))}>移除当前皮肤</button>}</div>
    <HistorySizeSetting/>
...
export function applySkins(ctx) {
  let runtime;
  ctx.effect(() => { runtime = createSkinRuntime(ctx, adapterCss, { 'ios-liquid': iosLiquidLayout, 'codex-desktop': codexDesktopLayout, 'claude-cli-terminal': claudeTerminalLayout, [MATERIAL_LAYOUT]: materialLayout }); return () => runtime.dispose(); });
