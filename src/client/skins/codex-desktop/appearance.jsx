...
export function CodexAppearance({ runtime, state, act, importFile, error }) {
  return <section className="tx-app omd-appearance codex-appearance">
    ...
    <div className="codex-setting-group">
      <label className="omd-appearance-row"><span>主题<small>选择浅色、深色或跟随系统</small></span>
    ...
    <div className="codex-setting-group">
      <label className="omd-appearance-row"><span>皮肤</span><select aria-label="皮肤" value={state.selected} onChange={e => act(() => runtime.select(e.target.value))}>
      ...
      <div className="omd-appearance-row"><span>导入皮肤<small>选择 .omd-skin.json 文件</small></span>
        <label className="codex-import">选择文件<input aria-label="导入皮肤" type="file" accept=".json,application/json" onChange={importFile}/></label>
      </div>
    </div>
    <h3>对话</h3>
    <div className="codex-setting-group"><HistorySizeSetting/></div>
    <div className="omd-appearance-actions">
      <button type="button" className="tx-button" onClick={() => act(() => runtime.reset())}>恢复默认皮肤</button>
      {!state.skins.find(s => s.id === state.selected)?.builtin && <button type="button" className="tx-button" onClick={() => act(() => runtime.remove(state.selected))}>移除当前皮肤</button>}
    </div>
