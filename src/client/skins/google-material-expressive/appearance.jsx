...
export function MaterialAppearance({ runtime, state, act, importFile, error }) {
  return <section className="gm-appearance omd-appearance">
    ...
    <div className="gm-setting-group">
      <label className="omd-appearance-row gm-setting-row"><MaterialIcon name="palette"/><span><strong>皮肤</strong><small>选择整个工作台的外观</small></span><select aria-label="皮肤" value={state.selected} onChange={e => act(() => runtime.select(e.target.value))}><option value="default">OMD 默认</option>{state.skins.map(s => <option key={s.id} value={s.id}>{s.name} · {s.version}</option>)}</select></label>
    ...
    <h2 className="gm-group-label">管理皮肤</h2>
    <div className="gm-setting-group">
      <label className="omd-appearance-row gm-setting-row gm-import"><MaterialIcon name="file_upload"/><span><strong>导入皮肤</strong><small>选择 .omd-skin.json 文件</small></span><input aria-label="导入皮肤" type="file" accept=".json,application/json" onChange={importFile}/></label>
      <div className="omd-appearance-actions"><button type="button" className="gm-tonal-button" onClick={() => act(() => runtime.reset())}>恢复默认皮肤</button>{!state.skins.find(s => s.id === state.selected)?.builtin && <button type="button" className="gm-text-button" onClick={() => act(() => runtime.remove(state.selected))}>移除当前皮肤</button>}</div>
    </div>
