...
const CONTEXT_UI_VERSION = '1.1.0';
const CONTEXT_FREQUENCY_PRESETS = Object.freeze({
...
function wrapContextClient(legacy, require) {
  ...
  class ContextSettings extends React.Component {
    ...
    renderAdvanced() {
      ...
      return h(React.Fragment, null, h('div', { className: 'cx-info' }, icon('info'), h('p', null, '频率档位可以直接使用。需要精调时再展开；这些选项均对应仍在运行的功能。')),
        fold('预处理窗口', '分段、参考前文与空闲冲刷', h('div', { className: 'cx-grid' }, this.number('digestEvery', '预处理频率 · 新事件数', 1), this.number('digestWindow', '目标分段窗口 · 事件数', 1, '保留完整工具往返，实际段长可能超过窗口。'), this.number('digestLookback', '参考前文 · 事件数', 0), this.number('flushIdleMs', '无新事件冲刷 · 秒', 0, '0 关闭；工具长时间运行也可能触发。', 1000))),
        ...
        fold('Trace 与资源上限', '0 使用原有默认语义，不自动裁切内容', h('div', { className: 'cx-grid' }, this.number('traceMaxChars', '推理文本字符上限', 0, '0 保留所选推理全文；正数保留尾部并标注截取。'), this.number('digestMaxTokens', '预处理输出 Token 上限', 0, '0 使用提供方默认。'), this.number('surgeonMaxTokens', '中枢输出 Token 上限', 0), this.number('jobTimeoutMs', '后台超时 · 秒', 0, '0 不设置插件超时。', 1000))),
...
return wrapContextClient(module.exports,require);}});
