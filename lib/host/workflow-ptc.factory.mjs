...
// node_modules/.pnpm/acorn@8.18.0/node_modules/acorn/dist/acorn.js
var require_acorn = __commonJS({
  "node_modules/.pnpm/acorn@8.18.0/node_modules/acorn/dist/acorn.js"(exports2, module2) {
...
// vendor/dsh/workflow-ptc/src/index.ts
var index_exports = {};
__export(index_exports, {
  MaterializeError: () => MaterializeError,
  PtcWorkflowRun: () => PtcWorkflowRun,
  WorkflowExecution: () => WorkflowExecution,
  WorkflowJournal: () => WorkflowJournal,
  WorkflowWorktree: () => WorkflowWorktree,
  default: () => index_default,
  materializeFromRealm: () => materializeFromRealm,
  parseWorkflowSource: () => parseWorkflowSource,
  readJournal: () => readJournal,
  requestKey: () => requestKey,
  runWorkflowGuest: () => runWorkflowGuest,
  serializeWorkflowSource: () => serializeWorkflowSource,
...
function compactGuestLocations(text2) {
  return text2.replace(/data:(?:text|application)\/(?:javascript|ecmascript)[^\s]+/g, (location) => {
    const tail = /(:\d+:\d+)(\)?)$/.exec(location);
    return `workflow-guest${tail ? tail[1] + tail[2] : ""}`;
  });
}
...
function workflowPaths(root, sessionId, id) {
  const directory = (0, import_node_path3.resolve)(root, (0, import_node_crypto2.createHash)("sha256").update(sessionId).digest("hex"), runId(id));
  return { directory, scriptPath: (0, import_node_path3.join)(directory, "script.js"), transcriptDir: directory };
}
function readJournal(text2) {
...
var WorkflowJournal = class _WorkflowJournal {
  constructor(directory, file, lease, sourceLease, cached) {
  ...
  static async create(root, sessionId, id, input) {
    const scope = (0, import_node_path3.join)(root, (0, import_node_crypto2.createHash)("sha256").update(sessionId).digest("hex"));
    await (0, import_promises2.mkdir)(scope, { recursive: true, mode: 448 });
    const canonical = await (0, import_promises2.realpath)(scope);
    let sourceLease;
    let lease;
    let file;
    try {
      let cached = /* @__PURE__ */ new Map();
      if (input.resumeFromRunId !== void 0) {
        if (input.resumeFromRunId === id) throw new Error("A workflow cannot resume itself");
        const source = (0, import_node_path3.join)(canonical, runId(input.resumeFromRunId));
        await (0, import_promises2.readFile)((0, import_node_path3.join)(source, "run.json"), "utf8");
        sourceLease = await SessionWriteLease.acquire(source, sessionId);
        cached = readJournal(await (0, import_promises2.readFile)((0, import_node_path3.join)(source, "journal.jsonl"), "utf8"));
      }
      const directory = (0, import_node_path3.join)(canonical, runId(id));
      await (0, import_promises2.mkdir)(directory, { mode: 448 });
      lease = await SessionWriteLease.acquire(directory, sessionId);
      for (const [name, contents] of [["script.js", input.script], ["run.json", JSON.stringify({ version: 1, id, sessionId, ...input }) + "\n"]]) {
        const handle = await (0, import_promises2.open)((0, import_node_path3.join)(directory, name), "wx", 384);
        try {
      ...
      file = await (0, import_promises2.open)((0, import_node_path3.join)(directory, "journal.jsonl"), "ax", 384);
      const journal = new _WorkflowJournal(directory, file, lease, sourceLease, cached);
  ...
  append(event) {
    ...
    const text2 = JSON.stringify(event) + "\n";
    this.chain = this.chain.then(async () => {
      await this.file.writeFile(text2);
      await this.file.sync();
    });
    return this.chain;
  }
  async start(seq, identity, valid = () => true) {
  ...
  async close(outcome) {
    this.closing ??= (async () => {
      try {
        await this.append({ type: "end", outcome });
      } finally {
        this.closed = true;
        try {
          ...
          try {
            ...
            await this.sourceLease?.release();
          }
        }
      }
    })();
...
// vendor/dsh/workflow-ptc/src/source.ts
var import_acorn = __toESM(require_acorn(), 1);

// vendor/dsh/workflow-ptc/src/meta.ts
var import_dsh_workflow = require("@deepseek-ai/dsh-workflow");
function validateMetaShape(meta) {
  ...
  const record = meta;
  const known = /* @__PURE__ */ new Set(["name", "description", "whenToUse", "phases"]);
  for (const key of Object.keys(record)) {
...
function validateMeta(value) {
  ...
  if (meta === void 0) {
    throw new import_dsh_workflow.WorkflowError(`invalid meta: ${violations.join("; ")}`, "META_INVALID");
  }
  return meta;
}
...
function parseWorkflowSource(script, suppliedMeta) {
  const tree = (0, import_acorn.parse)(script, { ecmaVersion: "latest", sourceType: "module", allowAwaitOutsideFunction: true, allowReturnOutsideFunction: true });
  const first = tree.body[0];
  if (first?.type !== "ExportNamedDeclaration") {
    if (suppliedMeta === void 0) throw new Error("Workflow script must begin with export const meta = {...}, or supply the meta field");
  ...
  if (suppliedMeta !== void 0 && JSON.stringify(meta) !== JSON.stringify(validateMeta(suppliedMeta))) throw new Error("Workflow inline meta and meta field disagree");
  return { meta, body: script.slice(0, first.start) + script.slice(first.start, first.end).replace(/[^\n\r]/g, " ") + script.slice(first.end) };
}
function serializeWorkflowSource(meta, body) {
  return `export const meta = ${JSON.stringify(meta, null, 2)};
${body}`;
}

// vendor/dsh/workflow-ptc/src/host.ts
var unquiescedJournals = /* @__PURE__ */ new Set();
var GUEST_URL = `data:text/javascript,${encodeURIComponent(WORKFLOW_GUEST_SOURCE)}`;
var PROGRAM = `const { runWorkflowGuest } = await import(${JSON.stringify(GUEST_URL)}); return await runWorkflowGuest(workflowHost);`;
...
function json(value) {
  const result = (0, import_dsh_util_values.snapshotJsonValue)(value);
...
function childRequest(value) {
  ...
  if (!Number.isSafeInteger(request.seq) || request.seq < 1) throw new Error("workflow child sequence must be a positive integer");
  const extras = {};
  for (const key of ["provider", "model", "effort", "agentType"]) {
    if (request[key] !== void 0) {
      const value2 = text(request[key], key);
      if (!value2.trim() || value2 !== value2.trim()) throw new Error(`workflow ${key} must be non-empty and normalized`);
      extras[key] = value2;
    }
  }
  if (request.isolation !== void 0 && request.isolation !== "worktree") throw new Error("workflow isolation must be worktree");
  let schema;
  if (request.schema !== void 0) {
    const candidate = object(request.schema);
    (0, import_dsh_tools.assertObjectJsonSchema)(candidate);
    schema = candidate;
  }
  return {
    seq: request.seq,
    ...
    ...schema === void 0 ? {} : { schema },
    ...extras,
    ...request.isolation === void 0 ? {} : { isolation: "worktree" }
  };
}
function agentInfo(value) {
  ...
  return {
    ...
    childId: (0, import_dsh_session.SessionId)(text(info.childId, "child id")),
...
function progress(value) {
  ...
  switch (event.type) {
    case "phase":
      return { type: "phase", title: text(event.title, "phase") };
    case "log":
      return { type: "log", message: text(event.message, "log") };
    case "agent-start":
      return { type: "agent-start", info: agentInfo(event.info) };
    case "agent-end": {
...
var PtcWorkflowRun = class {
  constructor(ctx, subagents, runtime, id, meta, parent, init, provider, policy, observer, signal, support) {
    this.ctx = ctx;
  ...
  get worktrees() {
    return [...this.artifacts.values()].map((value) => ({ ...value }));
  }
  /**
  ...
  bindings() {
    return {
      ...
      loadWorkflow: (value) => this.track((async () => {
        ...
        return json(await this.support.loadWorkflow(value, this.controller.signal));
      })()),
  ...
  async startChild(request) {
    ...
    const admission = this.admission.then(async () => {
      this.requireActive();
      const type2 = await this.support?.childType?.(request.agentType);
      const identity = { ...request, parentOptions: this.parent.session.requestHeader()?.config ?? this.parent.options, policy: this.policy, presetFingerprint: type2?.fingerprint };
      const cached2 = await this.journal?.start(callId, identity, (value) => !value.worktree?.retained || (0, import_node_fs.existsSync)(value.worktree.path));
      return { type: type2, cached: cached2 };
    });
    ...
    if (!cached && budget?.total !== null && budget?.total !== void 0 && budget.spent >= budget.total) throw new Error("Workflow token target reached; no new agents can start");
    if (cached) {
      if (cached.worktree) this.artifacts.set(callId, cached.worktree);
      const run2 = { id: (0, import_dsh_session.SessionId)(cached.childId ?? `cached-${this.id}-${callId}`), result: Promise.resolve(cached), dispose: async () => {
      } };
      this.children.set(callId, { callId, run: run2, cached: true });
      return { callId, childId: run2.id };
    }
    ...
    const run = await this.subagents.start(this.provider, {
      ...
      ...request.provider === void 0 && request.model === void 0 && request.effort === void 0 ? {} : {
        agentOptions: {
          ...request.provider === void 0 ? {} : { provider: request.provider },
          ...request.model === void 0 ? {} : { model: request.model },
          ...request.effort === void 0 ? {} : { reasoningEffort: request.effort }
        }
      }
    });
  ...
  onProgress(event) {
    ...
    switch (event.type) {
      ...
      default:
        (0, import_dsh_util_values.assertNever)(event, "workflow progress");
    }
  }
  endAgent(info) {
  ...
  async drive() {
    ...
    try {
      ...
      if (this.support) {
        this.journal = await WorkflowJournal.create(this.support.root, this.parent.session.id, this.id, {
          script: serializeWorkflowSource(this.meta, this.init.body),
          meta: this.meta,
          args: this.init.args,
      ...
      else if (outcome.error !== void 0) result = { value: null, stopReason: "error", error: `workflow execution failed (${outcome.error.kind}): ${outcome.error.message}`, agentsStarted: this.started };
      else result = workflowResult(outcome.value);
    } catch (error) {
      this.readiness.reject(error);
      this.terminal = true;
      result = this.cancelReason === void 0 ? { value: null, stopReason: "error", error: renderThrown(error), agentsStarted: this.started } : this.cancelled();
    } finally {
      this.terminal = true;
    ...
    try {
      await this.journal?.close(result.stopReason);
    } catch (error) {
      return { value: null, stopReason: "error", agentsStarted: this.started, error: `Workflow journal could not settle: ${renderThrown(error)}` };
    }
    return result;
  }
};
...
var import_dsh_tools2 = require("@deepseek-ai/dsh-tools");
...
var WorkflowWorktree = class _WorkflowWorktree {
  ...
  static async create(cwd, stateDirectory, policy, signal, confine) {
    ...
    const repository = await (0, import_promises3.realpath)(await runGit(cwd, ["rev-parse", "--show-toplevel"], signal));
    let filterKeys = "";
    try {
    ...
    const sourceCwd = await (0, import_promises3.realpath)(cwd);
    const subdirectory = (0, import_node_path4.relative)(repository, sourceCwd);
    if (subdirectory === ".." || subdirectory.startsWith(".." + import_node_path4.sep) || (0, import_node_path4.isAbsolute)(subdirectory) || (0, import_node_path4.resolve)(repository, subdirectory) !== sourceCwd) throw new Error("Workflow cwd is not inside the repository");
    const base = await runGit(repository, ["rev-parse", "--verify", "HEAD^{commit}"], signal);
    await (0, import_promises3.mkdir)(stateDirectory, { recursive: true, mode: 448 });
    const canonicalState = await (0, import_promises3.realpath)(stateDirectory);
    const id = (0, import_node_crypto3.randomUUID)(), path = (0, import_node_path4.join)(canonicalState, id), receiptPath = (0, import_node_path4.join)(canonicalState, id + ".json");
    const artifact = { path, cwd: (0, import_node_path4.join)(path, subdirectory), repository, base, retained: true, reason: "starting" };
    await (0, import_promises3.writeFile)(receiptPath, JSON.stringify(artifact) + "\n", { flag: "wx", mode: 384 });
  ...
  async save() {
    await (0, import_promises3.writeFile)(this.receiptPath, JSON.stringify(this.artifact) + "\n", { mode: 384 });
  }
...
var import_dsh_brand2 = require("@deepseek-ai/dsh-brand");
var import_dsh_tools3 = require("@deepseek-ai/dsh-tools");
var import_dsh_workflow2 = require("@deepseek-ai/dsh-workflow");
var SUPPORTED_AGENT_OPTIONS = /* @__PURE__ */ new Set(["label", "phase", "schema", "provider", "model", "effort", "isolation", "agentType"]);
function outputText(blocks) {
  return blocks.filter((block) => block.type === "text").map((block) => block.text).join("");
}
...
${body}
      ...
      throw new import_dsh_workflow2.WorkflowError(`workflow script does not parse: ${String(error)}`, "SCRIPT_PARSE", { cause: error });
    }
    this.context = vm.createContext({}, { name: `workflow:${meta.name}` });
    const sharedBudget = this.shared.budget;
    const globals = {
  ...
  materializeResult(raw) {
    try {
      ...
      if (!(error instanceof MaterializeError)) throw error;
      throw new import_dsh_workflow2.WorkflowError(
        `the workflow's return value is not plain JSON data \u2014 ${error.message}. Return only JSON-serializable objects/arrays/scalars.`,
        "RESULT_UNSERIALIZABLE",
        { cause: error }
      );
  ...
  async agent(rawPrompt, rawOpts) {
    if (typeof rawPrompt !== "string" || rawPrompt.length === 0) {
      throw new import_dsh_workflow2.WorkflowError("agent() requires a non-empty prompt string", "INVALID_ARGUMENT");
    }
    const opts = this.readAgentOptions(rawOpts);
    if (this.shared.started >= this.limits.maxTotalAgents) {
      throw new import_dsh_workflow2.WorkflowError(
    ...
    try {
      ...
      try {
        ...
        throw new import_dsh_workflow2.WorkflowError(`agent() could not start a child: ${renderThrown(error)}`, "AGENT_START", { cause: error });
      }
      const info = { seq, label, ...phase !== void 0 ? { phase } : {}, childId: (0, import_dsh_brand2.brandString)(run.id) };
      this.observer.agentStart(info);
      try {
        let result;
        try {
          ...
          this.observer.agentEnd({ ...info, outcome: "failed" });
          throw new import_dsh_workflow2.WorkflowError(`child agent run failed: ${renderThrown(error)}`, "AGENT_RESULT", { cause: error });
        }
        if (result.stopReason === "completed") {
          if (opts.schema !== void 0) {
            if (result.structured === void 0) {
  ...
  readAgentOptions(rawOpts) {
    ...
    try {
      ...
      if (!(error instanceof MaterializeError)) throw error;
      throw new import_dsh_workflow2.WorkflowError(`agent() options must be plain JSON data \u2014 ${error.message}`, "INVALID_ARGUMENT", { cause: error });
    }
    if (typeof opts !== "object" || opts === null || Array.isArray(opts)) {
      throw new import_dsh_workflow2.WorkflowError("agent() options must be an object", "INVALID_ARGUMENT");
    }
    const record = opts;
    for (const key of Object.keys(record)) {
      if (SUPPORTED_AGENT_OPTIONS.has(key)) continue;
      throw new import_dsh_workflow2.WorkflowError(`agent() option "${key}" is not recognized (supported: ${[...SUPPORTED_AGENT_OPTIONS].join(", ")})`, "UNSUPPORTED_OPTION");
    }
    for (const key of ["label", "phase", "provider", "model", "effort", "agentType"]) {
      if (record[key] !== void 0 && typeof record[key] !== "string") {
        throw new import_dsh_workflow2.WorkflowError(`agent() option "${key}" must be a string`, "INVALID_ARGUMENT");
      }
    }
    for (const key of ["provider", "model", "effort", "agentType"]) {
      if (record[key] !== void 0 && (!record[key].trim() || record[key] !== record[key].trim())) throw new import_dsh_workflow2.WorkflowError(`agent() option "${key}" must be non-empty and normalized`, "INVALID_ARGUMENT");
    }
    if (record.isolation !== void 0 && record.isolation !== "worktree") throw new import_dsh_workflow2.WorkflowError('agent() isolation must be "worktree"', "INVALID_ARGUMENT");
    let schema;
    if (record.schema !== void 0) {
      try {
        (0, import_dsh_tools3.assertObjectJsonSchema)(record.schema);
        schema = record.schema;
      } catch (error) {
        if (!(error instanceof import_dsh_tools3.JsonSchemaError)) throw error;
        throw new import_dsh_workflow2.WorkflowError(`agent() schema is outside the supported subset \u2014 ${error.message}`, "UNSUPPORTED_SCHEMA", { cause: error });
      }
    }
    return {
      ...record.label !== void 0 ? { label: record.label } : {},
  ...
  async workflow(rawReference, rawArgs) {
    if (this.depth !== 0) throw new import_dsh_workflow2.WorkflowError("workflow() nesting is limited to one level", "INVALID_ARGUMENT");
    const ref = materializeFromRealm(rawReference, "workflow reference");
    if (!(typeof ref === "string" && ref.length > 0) && !(ref && typeof ref === "object" && !Array.isArray(ref) && Object.keys(ref).length === 1 && typeof ref.scriptPath === "string" && ref.scriptPath.length > 0)) throw new import_dsh_workflow2.WorkflowError("workflow() requires a saved name or {scriptPath}", "INVALID_ARGUMENT");
    if (!this.children.loadWorkflow) throw new import_dsh_workflow2.WorkflowError("Saved workflows are unavailable", "INVALID_ARGUMENT");
    const input = rawArgs === void 0 ? void 0 : materializeFromRealm(rawArgs, "workflow args");
    const source = await this.children.loadWorkflow(ref);
    const prefix = `\u25B8 ${source.meta.name}`;
    const observer = {
    ...
    const result = await new _WorkflowExecution(source.meta, source.body, input, this.limits, observer, this.children, this.shared, 1).drive();
    if (result.stopReason !== "completed") throw new import_dsh_workflow2.WorkflowError(result.error ?? "Nested workflow failed", "AGENT_RESULT");
    return result.value;
  }
  /** The `parallel(thunks)` hook: each thunk caught → `null`; fatal errors propagate. */
  async parallel(rawThunks) {
    if (!Array.isArray(rawThunks)) {
      throw new import_dsh_workflow2.WorkflowError("parallel() requires an array of zero-argument functions", "INVALID_ARGUMENT");
    }
    this.assertItemCap(rawThunks.length, "parallel()");
    const thunks = rawThunks.map((thunk, index) => {
      if (typeof thunk !== "function") {
        throw new import_dsh_workflow2.WorkflowError(`parallel() item ${index} is not a function`, "INVALID_ARGUMENT");
      }
      return thunk;
    });
    return Promise.all(thunks.map(async (thunk) => {
      try {
        ...
        if ((0, import_dsh_workflow2.isFatalWorkflowError)(error)) throw error;
        return null;
      }
    }));
  }
  ...
  async pipeline(rawItems, rawStages) {
    if (!Array.isArray(rawItems)) {
      throw new import_dsh_workflow2.WorkflowError("pipeline() requires an items array", "INVALID_ARGUMENT");
    }
    this.assertItemCap(rawItems.length, "pipeline()");
    if (rawStages.length === 0) {
      throw new import_dsh_workflow2.WorkflowError("pipeline() requires at least one stage function", "INVALID_ARGUMENT");
    }
    const stages = rawStages.map((stage, index) => {
      if (typeof stage !== "function") {
        throw new import_dsh_workflow2.WorkflowError(`pipeline() stage ${index} is not a function`, "INVALID_ARGUMENT");
      }
      return stage;
    });
    return Promise.all(rawItems.map(async (item, index) => {
  ...
  phase(title) {
    if (typeof title !== "string" || title.length === 0) {
      throw new import_dsh_workflow2.WorkflowError("phase() requires a non-empty title string", "INVALID_ARGUMENT");
    }
    this.currentPhase = title;
    this.observer.phase(title);
  }
  ...
  log(message) {
    if (typeof message !== "string") {
      throw new import_dsh_workflow2.WorkflowError("log() requires a message string", "INVALID_ARGUMENT");
    }
    this.observer.log(message);
  }
};
...
function assertBodyParses(body, name) {
  if (META_STATEMENT.test(body)) {
    throw new import_dsh_workflow3.WorkflowError("workflow meta rides the `meta` request field, not the script: remove the `export const meta = {...}` statement from the body", "SCRIPT_PARSE");
  }
  try {
    void new vm2.Script(`(async () => {
${body}
})()`, { filename: `workflow:${name}`, lineOffset: -1 });
  } catch (error) {
    throw new import_dsh_workflow3.WorkflowError(`workflow script does not parse: ${String(error)}`, "SCRIPT_PARSE", { cause: error });
  }
}
function resolveSubagentProvider(ctx, configured, override) {
  ...
  if (provider.length === 0 || provider !== provider.trim()) {
    throw new import_dsh_workflow3.WorkflowError(
      "workflow subagentProvider must be a non-empty normalized string",
      "INVALID_ARGUMENT"
    );
  }
  if (ctx.subagents.getProvider(provider) === void 0) {
    throw new import_dsh_workflow3.WorkflowError(`no subagent provider registered for "${provider}"`, "AGENT_START");
  }
  return provider;
}
function resolveMaxTotalAgents(requested, ceiling) {
  ...
  if (!Number.isSafeInteger(requested) || requested < 1) {
    throw new import_dsh_workflow3.WorkflowError("workflow maxTotalAgents must be a positive safe integer", "INVALID_ARGUMENT");
  }
...
var PtcWorkflowEngine = class extends import_dsh_workflow3.WorkflowEngine {
  static inject = ["subagents", "ptcRuntime", "sandboxPolicy"];
  static Config = import_schemastery.default.object({
    stateDirectory: import_schemastery.default.string(),
    provider: import_schemastery.default.string().default("spawn"),
    maxConcurrentAgents: import_schemastery.default.natural().default(0),
    maxTotalAgents: import_schemastery.default.natural().min(1).default(1e3),
    maxItemsPerCall: import_schemastery.default.natural().min(1).default(4096),
    syncTimeoutMs: import_schemastery.default.natural().min(1).default(5e3)
  });
  config;
  ...
  async prepare(input, parent, signal) {
    if (["script", "name", "scriptPath"].filter((key) => input[key] !== void 0).length !== 1) throw new Error("Supply exactly one of script, name, or scriptPath");
    ...
    if (input.meta !== void 0) throw new Error("Saved workflow metadata belongs in its script file");
    return { script: source.body, meta: source.meta };
  }
  async loadSource(reference, parent, signal) {
    ...
    if (typeof reference === "string") {
      ...
      path = (0, import_node_path5.join)(".omd", "workflows", reference + ".js");
  ...
  start(request) {
    ...
    const id = (0, import_dsh_workflow3.WorkflowRunId)((0, import_node_crypto4.randomUUID)());
    const info = { id, meta };
    const limits = {
      maxConcurrentAgents: this.config.maxConcurrentAgents === 0 ? Math.min(16, Math.max(1, (0, import_node_os.availableParallelism)() - 2)) : this.config.maxConcurrentAgents,
      maxTotalAgents,
    ...
    const budget = runCtx.get("trisoulX")?.workflowBudget?.capture(request.parent.session);
    const run = new PtcWorkflowRun(
      ...
      request.signal,
      {
        root: this.config.stateDirectory ?? (0, import_node_path5.join)(this.ctx.get("trisoulX")?.store.dir ?? (0, import_node_path5.join)(process.env.DSH_HOME ?? (0, import_node_path5.join)((0, import_node_os.homedir)(), ".dsh"), "trisoul-x"), "workflows"),
...
var index_default = PtcWorkflowEngine;
