...
// vendor/dsh/tool-bash/src/index.ts
var index_exports = {};
...
var import_dsh_shell2 = require("@deepseek-ai/dsh-shell");

// vendor/dsh/tool-bash/src/background.ts
...
// vendor/dsh/tool-bash/src/render.ts
...
function bashDescription(backgroundEnabled, escalationModes) {
  ...
  const base = `Execute a bash command (\`bash -c\`) and return its stdout/stderr. Each call runs in a fresh shell: no state (cwd, variables, functions) persists between calls \u2014 pass \`workdir\` instead of using \`cd\`. Non-zero exits are reported as \`[exit code: N]\`. Current harness environment facts are exposed through managed \`$${import_dsh_shell2.DSH_ENV_PREFIX}*\` variables; inspect them when needed. Commands may run under a file sandbox; a blocked file operation is reported as \`[sandbox: file access denied under <mode> mode]\` \u2014 a policy denial, not a bug in the command; do not retry another way. Long output is truncated to its tail; the full output is saved to a file whose path is reported when available. ` + background;
...
function resolveWorkdir(modelWorkdir, exec, policyWorkspaceRoot) {
  ...
  if (sessionCwd !== void 0 && !(0, import_node_path.isAbsolute)(modelWorkdir)) {
    return `${sessionCwd}${import_node_path.sep}${modelWorkdir}`;
  }
  return modelWorkdir;
}
...
function apply(ctx, config = {}) {
  ...
  ctx.tools.register((0, import_dsh_tools.defineTool)({
    ...
    parameters: {
      ...
      ...backgroundEnabled ? { yieldMs: { type: "number", description: "Initial wait in milliseconds (250\u201330000) for enabled automatic background mode on top-level calls only. Explicit foreground and SDK calls keep their final-result semantics." } } : {},
      ...
      if (args.run_in_background === true) {
        ...
        const id = jobs2.start({
          ...
          run: () => processJob(
            ...
            (proc) => proc.peekOutput ? renderProcessRead(proc.peekOutput(), proc.sandbox, escalationModes) : "[Output preview unavailable; use job_output.]"
          )
        });
      ...
      if (backgroundEnabled && exec.agent && !exec.parent && args.run_in_background === void 0 && softYield !== void 0) {
        const result2 = await foregroundOrJob(
          ...
          (value) => ({
            ...
            output: renderResult(value, escalationModes)
          })
        );
        ...
        return { kind: "foreground", ...canonicalBashResult(result2.value) };
      }
