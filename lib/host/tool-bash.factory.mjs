...
function validateBashArgs(args, effectiveMode) {
  ...
  (0, import_dsh_sandbox3.validateEscalationArgs)(args.sandbox_permissions, justification);
}
...
function apply(ctx, config = {}) {
  ...
  const bashTool = (jobs) => {
    ...
    const waitOnJob = async (registry, attached, exec, spec, waitMs = spec.timeoutMs, softYield = false) => {
      ...
      return { kind: "foreground", ...canonicalBashResult(result), ...stopped !== void 0 ? { stopped } : {} };
    };
  ...
  ctx.inject(["jobs"], (jobCtx) => {
    ...
    jobCtx.effect(() => () => {
      unregister();
      if (ctx.fiber.state === import_cordis.FiberState.ACTIVE) foregroundOnly = ctx.tools.register(bashTool(void 0));
    });
  });
}
