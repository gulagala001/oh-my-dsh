...
// vendor/dsh/tools/src/index.ts
var index_exports = {};
__export(index_exports, {
  CodeRunFailedError: () => CodeRunFailedError,
  JsonSchemaError: () => JsonSchemaError,
  RUN_CODE_NAME: () => RUN_CODE_NAME,
  ...
  defineContentToolFixture: () => defineContentToolFixture,
  defineTool: () => defineTool,
  jsonSchemaToPy: () => jsonSchemaToPy,
  jsonSchemaToTs: () => jsonSchemaToTs,
  parameterSchemaSpecToJsonSchema: () => parameterSchemaSpecToJsonSchema,
  renderToolsSdk: () => renderToolsSdk,
  renderToolsSdkPy: () => renderToolsSdkPy,
...
var import_dsh_llm4 = require("@deepseek-ai/dsh-llm");
var import_dsh_util_values3 = require("@deepseek-ai/dsh-util-values");
...
var JsonSchemaError = class extends import_dsh_llm.HarnessError {
...
function scalarMatches(type, value) {
  switch (type) {
    ...
    default:
      return (0, import_dsh_util_values.assertNever)(type, "JsonSchemaType");
  }
}
  ...
  for (let task = tasks.pop(); task !== void 0; task = tasks.pop()) {
    ...
    for (const key of Object.keys(node)) {
      ...
      if (ANNOTATION_KEYWORDS.has(key)) {
        try {
          if (!(0, import_dsh_util_values.isJsonValue)(node[key])) violations.push(`${path}.${key} annotation must be lossless JSON data`);
        } catch {
    ...
    switch (schemaType) {
      ...
      default:
        (0, import_dsh_util_values.assertNever)(schemaType, "JsonSchemaType");
    }
  }
}
...
function safelyIsJsonValue(value) {
  try {
    return (0, import_dsh_util_values.isJsonValue)(value);
  } catch {
    return false;
  }
}
    ...
    try {
      ...
      switch (nodeType) {
        ...
        case "null":
          ...
          break;
        default:
          finish((0, import_dsh_util_values.assertNever)(nodeType, "JsonSchemaType"));
      }
    } catch (error) {
...
// vendor/dsh/tools/src/ptc.ts
var import_dsh_brand = require("@deepseek-ai/dsh-brand");
var import_dsh_llm3 = require("@deepseek-ai/dsh-llm");
var import_dsh_sandbox = require("@deepseek-ai/dsh-sandbox");
var import_dsh_util_values2 = require("@deepseek-ai/dsh-util-values");

// vendor/dsh/tools/src/schema.ts
var import_dsh_llm2 = require("@deepseek-ai/dsh-llm");
...
var ToolArgsError = class extends import_dsh_llm2.HarnessError {
...
var RUN_CODE_CONTROLS = {
  ...
  sandbox_permissions: { type: "string", enum: [...import_dsh_sandbox.ESCALATION_TARGETS], description: "Wider sandbox mode for this complete program execution; requires justification and approval." },
...
var CodeRunFailedError = class extends import_dsh_llm3.HarnessError {
  constructor(message) {
...
function jsonNormalizeArgs(value) {
  let snapshot;
  try {
    snapshot = (0, import_dsh_util_values2.snapshotJsonValue)(value);
  } catch (error) {
  ...
  const logged = (0, import_dsh_util_values2.snapshotJsonValue)(snapshot);
  if (logged === void 0) {
...
function createRunCodeTool(registry, options) {
  ...
  const definition = defineTool({
    ...
    async execute(args, exec) {
      ...
      (0, import_dsh_sandbox.validateEscalationArgs)(args.sandbox_permissions, args.justification);
      if (args.timeoutMs !== void 0 && runtime.timeout === void 0) {
      ...
      if (args.sandbox_permissions !== void 0 && args.justification !== void 0) {
        ...
        const approvedMode = await (0, import_dsh_sandbox.approveEscalation)({
      ...
      const binding = (schema) => async (rawArgs) => {
        ...
        const n = ++dispatches;
        const subCallId = (0, import_dsh_brand.brandString)(`${String(exec.callId)}:ptc:${n}`);
        const input = {
          callId: subCallId,
          ...
          pendingQueue.push({
            ...
            async commit() {
              ...
              if (!result.isError && result.content.some((block) => block.type === "image")) {
                exec.deferContext((0, import_dsh_llm3.createUserMessage)({
                  content: result.content,
      ...
      for (const schema of registry.schemas(exec.agent)) {
        ...
        Object.defineProperty(functions, schema.name, { enumerable: true, value: binding((0, import_dsh_util_values2.deepFreeze)(schema)) });
      }
      try {
        let result;
        try {
...
// vendor/dsh/tools/src/ts-types.ts
...
// vendor/dsh/tools/src/py-types.ts
var IDENTIFIER2 = new RegExp("^[\\p{XID_Start}_]\\p{XID_Continue}*$", "u");
...
// vendor/dsh/tools/src/testing.ts
...
var ToolNotFoundError = class extends import_dsh_llm4.HarnessError {
  /**
...
var ToolOutputError = class extends import_dsh_llm4.HarnessError {
...
function snapshotProjection(toolName, projector, candidate) {
  try {
    const detached = (0, import_dsh_util_values3.snapshotJsonValue)(candidate);
    if (detached === void 0) {
...
function materializePresentation(candidate) {
  ...
  return (0, import_dsh_util_values3.deepFreeze)(detached);
}
function errorInfo(error) {
  try {
    return error instanceof import_dsh_llm4.HarnessError ? { name: error.name, code: error.code } : void 0;
  } catch {
    return void 0;
  }
}
var ToolLayer = class {
  tools;
  restrictions = new import_dsh_scope.AnonymousEntries();
  guards = new import_dsh_scope.AnonymousEntries();
  /**
  ...
  directTools;
  constructor(scope) {
    this.tools = new import_dsh_scope.NamedEntries((name) => new Error(scope === void 0 ? `tool "${name}" is already registered (for a per-agent variant, register through that agent's \`agent.ctx\` instead)` : `tool "${name}" is already registered in this scope`));
  }
...
var ToolRuntime = class extends import_cordis.Service {
  static inject = ["systemPrompt"];
  static Config = import_schemastery.default.object({
    mode: import_schemastery.default.union(["native", "ptc", "both"]).default("native"),
    maxParallelSubCalls: import_schemastery.default.natural().min(1).default(10)
  });
  ...
  layers = new import_dsh_scope.ScopedLayers(
  ...
  directSnapshots = /* @__PURE__ */ new WeakMap();
  defaultMode;
  maxParallelSubCalls;
  /**
  ...
  collapseSection() {
    return {
      ...
      text: (context) => this.modeFor(context.scope) === "ptc" ? this.directFor(context.scope).length ? `Use run_code and the tools SDK for business operations. These control tools are also callable directly: ${this.directFor(context.scope).join(", ")}. All other tools require run_code.` : PTC_ONLY_INSTRUCTION : ""
    };
  }
  /**
   ...
   */
  presentAs(mode, options = {}) {
    const ctx = this.ctx;
    if ((0, import_dsh_scope.scopeOf)(ctx) === void 0) {
  ...
  directFor(scope, refresh = false) {
    if (!scope) return [];
    if (!refresh) {
      const snapshot2 = this.directSnapshots.get(scope);
      if (snapshot2) return snapshot2;
    }
    const layer = this.layers.chainLayers(scope).findLast((layer2) => layer2.mode !== void 0);
   ...
   */
  restrict(filter) {
    const scope = (0, import_dsh_scope.scopeOf)(this.ctx);
    if (scope === void 0) {
  ...
  sdkSchemas(scope) {
    return [...this.view(scope).visible.values()].filter((definition) => definition.name !== RUN_CODE_NAME).map((definition) => {
      const output = (0, import_dsh_util_values3.snapshotJsonValue)(definition.output.schema);
      if (output === void 0) {
  ...
  schemaOf(definition, detachParameters) {
    ...
    const detached = detachParameters ? (0, import_dsh_util_values3.snapshotJsonValue)(parameters) : parameters;
    if (detached === void 0) {
  ...
  async shapeDispatchLog(dispatch) {
    try {
      return await this.ctx.waterfall(
        (0, import_dsh_scope.scopeTarget)(this, dispatch.agent),
  ...
  async completeScheduledExecution(prepared) {
    switch (prepared.kind) {
      ...
      default:
        return (0, import_dsh_util_values3.assertNever)(prepared, "scheduled tool preparation");
    }
  }
  createExecution(exec) {
    ...
    try {
      const detached = (0, import_dsh_util_values3.snapshotJsonValue)(exec.arguments);
      if (detached === void 0) {
        throw new TypeError("tool execution arguments must be losslessly JSON-serializable");
      }
      const execution = { ...base, arguments: (0, import_dsh_util_values3.deepFreeze)(detached) };
  ...
  async prepareExecution(input, next) {
    ...
    try {
      const carrier = (0, import_dsh_scope.scopeTarget)(this, exec.agent);
      ...
      if (denialReason !== void 0) {
        return await next({
          ...
          result: this.materializeFinalResult({
            ...
            isError: true,
            error: { message: denialReason, ...denialInfo === void 0 ? {} : { info: denialInfo } }
          })
        });
      }
      if (this.callerCancelled(exec)) {
  ...
  async finalizeScheduledExecution(exec, result) {
    try {
      ...
      const projected = content === void 0 ? result : this.markCanonical(exec, this.materializeFinalResult({ ...result, content }));
  ...
  notifyResult(exec, result) {
    ...
    const callbacks = this.ctx.events.dispatch("emit", [
      (0, import_dsh_scope.scopeTarget)(this, exec.agent),
      "tools/result",
      exec,
      result
    ]);
  ...
  async serviceAsk(exec, ask) {
    ...
    switch (outcome) {
      ...
      default:
        return (0, import_dsh_util_values3.assertNever)(outcome, "ApprovalOutcome");
    }
  }
  /**
  ...
  createSuccessResult(exec, tool, candidate) {
    ...
    const value = (0, import_dsh_util_values3.deepFreeze)(detached);
    let rendered;
    try {
  ...
  materializeFinalResult(result) {
    ...
    return (0, import_dsh_util_values3.deepFreeze)({ ...detached, value: result.value });
  }
};
