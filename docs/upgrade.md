...
**旧会话“历史加载失败”的 V0 兼容修复尚未发布，当前 `v1.7.3` 不包含。** 遇到这类问题先看[故障处理](#legacy-history-failure)，不要把下方安装 1.7.3 的步骤当成修复方法。
...
| 早期 V0 会话显示“历史加载失败” | 部分旧字段和压缩记录需要额外兼容处理；**此修复待发布，1.7.3 不包含**。按[故障处理](#legacy-history-failure)保留原件并核对修复版本。 |
...
4. **安装桌面和 OMD，随后完整重启。** 使用上方配对的桌面版本，在应用内安装 OMD 1.7.3；若已确认遇到下方 V0 兼容问题，1.7.3 不能完成该修复，先按[故障处理](#legacy-history-failure)保留数据，待包含修复的版本发布后再验证。**以前用过 OMD 且仍有旧格式会话时，在 OMD 安装、启用并重启完成前，不要打开或继续旧会话**，以免原生宿主先转换会话、而 OMD 关联记录尚未同步。不要把整个 Web profile、`node_modules` 或锁文件复制覆盖 `profiles/desktop`；它由桌面应用管理。
...
**发布状态：修复待发布，现有 `v1.7.3` 不包含。** 本地已修复的环境不能代表其他用户从公开 tag 安装后也已获得修复。不要反复重装 1.7.3，或仅根据版本面板的“最新版本”判断问题已解决。
...
- **更早的 V0 会话：** 额外兼容修复待发布，现有 `v1.7.3` 不包含。错误特征、备份、修复版本确认及重启后验证见[“历史加载失败”故障处理](#legacy-history-failure)。
