...
| 组件 | 本手册目标 |
| --- | --- |
| Oh My DSH | **1.7.4**，Git tag `v1.7.4` |
...
**旧会话“历史加载失败”的 V0 兼容修复尚未发布，当前 `v1.7.4` 不包含。** 遇到这类问题先看[故障处理](#legacy-history-failure)，不要把下方安装 1.7.4 的步骤当成修复方法。

完成意味着：**首次安装能启动所选桌面应用或 Web、加载插件并显示当前版本 1.7.4；升级时原环境已备份，旧模型／会话／皮肤经过核对。** 同一安装方式升级沿用原数据目录和 profile（Web 还需保留原端口）；Web 转桌面则按[迁移专节](#web-to-desktop)接入数据，并切换到桌面独立的 `desktop` profile。GitHub 最新版本、下载成功或文件已改，均不能代替实际安装和重启。没有模型凭据时如实报告对话验证尚未完成。

<a id="desktop"></a>

## 桌面版：从零安装与升级

桌面版由 **DSH 官方桌面应用 + Oh My DSH 插件** 组成。本手册配对 **DSH 0.1.7-rc.2 + OMD 1.7.4**，由桌面应用管理内置宿主；无需克隆本仓库或运行下方 Web 启动命令。
...
```text
github:gulagala001/oh-my-dsh#v1.7.4
```
...
1. 桌面宿主为 **0.1.7-rc.2**，插件版本面板的「当前版本」为 **1.7.4**；「最新版本」不能代替当前版本。
...
若 AI 只有终端权限、无法操作桌面应用，就提供应用内安装和重启的具体步骤，由用户完成；不要绕过桌面插件管理器。完成本节后无需执行下方 Web 安装命令。[桌面验证范围](release-1.7.4.md#验证范围)。
...
| 早期 V0 会话显示“历史加载失败” | 部分旧字段和压缩记录需要额外兼容处理；**此修复待发布，1.7.4 不包含**。按[故障处理](#legacy-history-failure)保留原件并核对修复版本。 |
...
4. **安装桌面和 OMD，随后完整重启。** 使用上方配对的桌面版本，在应用内安装 OMD 1.7.4；若已确认遇到下方 V0 兼容问题，1.7.4 不能完成该修复，先按[故障处理](#legacy-history-failure)保留数据，待包含修复的版本发布后再验证。**以前用过 OMD 且仍有旧格式会话时，在 OMD 安装、启用并重启完成前，不要打开或继续旧会话**，以免原生宿主先转换会话、而 OMD 关联记录尚未同步。不要把整个 Web profile、`node_modules` 或锁文件复制覆盖 `profiles/desktop`；它由桌面应用管理。
...
**发布状态：修复待发布，现有 `v1.7.4` 不包含。** 本地已修复的环境不能代表其他用户从公开 tag 安装后也已获得修复。不要反复重装 1.7.4，或仅根据版本面板的“最新版本”判断问题已解决。
...
dsh --version
dsh plugin --profile web add github:gulagala001/oh-my-dsh#v1.7.4
dsh --profile web
```
...
- **更早的 V0 会话：** 额外兼容修复待发布，现有 `v1.7.4` 不包含。错误特征、备份、修复版本确认及重启后验证见[“历史加载失败”故障处理](#legacy-history-failure)。
...
1. 服务从原入口正常启动，日志无阻止运行的错误；宿主实际为 0.1.7-rc.2，插件当前版本为 1.7.4。
