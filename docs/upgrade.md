...
| 组件 | 本手册目标 |
| --- | --- |
| Oh My DSH | **0.1.7-rc.2.7**，Git tag `v0.1.7-rc.2.7` |
| DSH Web／官方桌面宿主 | **0.1.7-rc.2** |
| 内置 OpenCU | **1.1.4**，无需另装 |
| Web／源码环境 | Node.js ≥22.19、Git、pnpm 11.23.0；Windows 使用 PowerShell 7 |

版本号跟随完整 DSH 宿主版本，末尾数字表示插件补丁：`0.1.7-rc.2.4` → `0.1.7-rc.2.7`。旧 1.x 更新检查器无法自动识别本次编号切换；即使显示“已是最新版”，也需按本手册的固定 tag 手动更新一次。历史 tag 与安装包保持原样。
...
完成意味着：**首次安装能启动所选桌面应用或 Web、加载插件并显示当前版本 0.1.7-rc.2.7；升级时原环境已备份，旧模型／会话／皮肤经过核对。** 同一安装方式升级沿用原数据目录和 profile（Web 还需保留原端口）；Web 转桌面则按[迁移专节](#web-to-desktop)接入数据，并切换到桌面独立的 `desktop` profile。GitHub 最新版本、下载成功或文件已改，均不能代替实际安装和重启。没有模型凭据时如实报告对话验证尚未完成。

<a id="desktop"></a>

## 桌面版：从零安装与升级

桌面版由 **DSH 官方桌面应用 + Oh My DSH 插件** 组成。本手册配对 **DSH 0.1.7-rc.2 + OMD 0.1.7-rc.2.7**，由桌面应用管理内置宿主；无需克隆本仓库或运行下方 Web 启动命令。
...
1. 桌面宿主为 **0.1.7-rc.2**，插件版本面板的「当前版本」为 **0.1.7-rc.2.7**；「最新版本」不能代替当前版本。
...
若 AI 只有终端权限、无法操作桌面应用，就提供应用内安装和重启的具体步骤，由用户完成；不要绕过桌面插件管理器。完成本节后无需执行下方 Web 安装命令。[桌面验证范围](release-0.1.7-rc.2.7.md#验证范围)。
...
4. **安装桌面和 OMD，随后完整重启。** 使用上方配对的桌面版本，在应用内安装 OMD 0.1.7-rc.2.7；若遇到下方 V0 兼容问题，先按[故障处理](#legacy-history-failure)保留数据，再验证旧会话。**以前用过 OMD 且仍有旧格式会话时，在 OMD 安装、启用并重启完成前，不要打开或继续旧会话**，以免原生宿主先转换会话、而 OMD 关联记录尚未同步。不要把整个 Web profile、`node_modules` 或锁文件复制覆盖 `profiles/desktop`；它由桌面应用管理。
...
git switch --detach v0.1.7-rc.2.7
...
1. 服务从原入口正常启动，日志无阻止运行的错误；宿主实际为 0.1.7-rc.2，插件当前版本为 0.1.7-rc.2.7。
