...
在 **设置 → Oh My DSH → 实验性功能** 中配置“向模型提供运行状态”或“优化后台任务与等待”。运行状态默认关闭，后台优化默认开启；同页的 CoT 前置默认开启，CFR 默认关闭。保存后从下一次模型请求生效，已有会话也可使用。Todo 遇到阻塞可暂停本回合催促，不会误勾选任务；新回合恢复检查。[完整说明与调用示例](runtime-state-background.md)。
